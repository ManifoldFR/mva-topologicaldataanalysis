...
/// \author Wilson Jallet <wilson.jallet@polytechnique.org>
/// \date 20/10/2019
///
/// \brief TTK %bettiNumbers processing package.
///
/// %BettiNumbers is a TTK processing package that takes a scalar field on the input
/// and produces a scalar field on the output.
///
/// \sa ttk::Triangulation
/// \sa ttkBettiNumbers.cpp %for a usage example.
...
*/

  {
    ...
    msg << "[BettiNumbers] Data-set (" << vertexNumber << " points) processed in "
        << t.getElapsedTime() << " s. (" << threadNumber_ << " thread(s))."
        << std::endl;
    dMsg(std::cout, msg.str(), timeMsg);
  }

  // Initialize a set of UF objects for each vertex
  std::vector<UnionFind> vtxUfs(vertexNumber);

  // Iterate over the edges of the PL manifold
  auto edges = triangulation_->getEdges();

  std::cout << "Length of edges vec: " << edges->size() << std::endl;

  for(auto ed = edges->begin(); ed != edges->end(); ed++) {
    // the elements of the pair v are vertex Ids
    // we grab their UF elements and unite them
    auto& v1 = vtxUfs[ed->first];
    auto& v2 = vtxUfs[ed->second];
    makeUnion(&v1, &v2);
  }
